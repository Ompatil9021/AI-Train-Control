...
def get_explanation():
    try:
        data=request.get_json(); ahead_train=data.get('ahead_train'); behind_train=data.get('behind_train')
        if not ahead_train or not behind_train: return jsonify({"error": "Missing train data"}), 400
...
def respond_to_decision():
    ...
    with simulation.lock:
        ...
        if decision == 'accept':
            ...
            # This is the correct way to set this variable
